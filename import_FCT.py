...
with open('fct.csv', 'r') as f:
    ...
    print(b)
    header = next(b)
    sql = 'insert into app_FCT (FCT_id, food_grp_id, food_item_id,Food_grp,Food_name,Crop_ref,Edible,Energy,WATER,Protein,Fat,Carbohydrate,Fiber,ASH,CA,FE,MG,P,K,NA,ZN,CU,VITA_RAE,RETOL,B_Cart_eq,VITD,VITE,THIA,RIBF,NIA,VITB6C,FOL,VITB12,VITC) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
    for t in b:
        # tableに各行のデータを挿入する。
        data = t
#        print(t)
